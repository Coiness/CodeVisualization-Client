...
export function useCodeEditor(theme: "show" | "vs", language?: ShowCodeLanguage, minimapEnable?: boolean) {
  ...
  }, [dom.current, theme, language, minimapEnable]); // todo 想办法移除这里的 dom.current
  ...
  const setHeightLine = useCallback((row: number) => {
    ...
    let targetTop = Math.max(row - 1 - ScrollJGRowCount, 0) * EditerLineHeight;
    ...
    editor.current.setScrollTop(targetScroll, monaco.editor.ScrollType.Smooth);
  }, []);
